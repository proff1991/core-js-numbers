...
function getLastDigit(value) {
  return value < 10 ? value : value % (Math.trunc(value / 10) * 10);
}
...
function roundToPowerOfTen(num, pow) {
  const nummer = 10 ** pow;
  return Math.round(num / nummer) * nummer;
}
...
function toNumber(value, def) {
  ...
  return Number.isNaN(newNum) ? def : newNum;
}
...
function getFibonacciNumber(index) {
  const fib = [0, 1];
  for (let i = 2; fib.length <= index; i += 1) {
